...
class RegistrationForm(Form):
    firstname = StringField('First Name:', [validators.DataRequired(),validators.Length(min=1, max=25,message="First Name must be between 1 and 25 characters long.")])
    lastname = StringField('Last Name:', [validators.DataRequired(),validators.Length(min=1, max=25,message="Last Name must be between 1 and 25 characters long.")])
    ...
    time = StringField('Time:', [validators.Length(min=0, max=25,message="time must be between 1 and 25 characters long.")])
    restaurant_location = StringField('Location:', [validators.Length(min=0, max=200,message="time must be between 1 and 25 characters long.")])
